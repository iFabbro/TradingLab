...
def _strategy(name: str = "correctness") -> TimeSignalStrategy:
    return TimeSignalStrategy(
        StrategyConfig(name=name, universe=["DEMO"], lookback=2),
        [],
    )
...
def test_signal_is_not_allowed_to_trade_on_same_bar(tmp_path):
    ...
    strategy = TimeSignalStrategy(
        StrategyConfig(name="lookahead", universe=["DEMO"], lookback=2),
        [1.0, 1.0, 1.0],
    )

    result = BacktestEngine(output_dir=tmp_path).run(prices, strategy)

    # The signal at t=0 cannot capture the t=0 -> t=1 move twice or earlier.
    # Entry is at the close of t=0 and the first return is earned on t=1.
    ...
    assert result.trade_log.loc[0, "entry_price"] == pytest.approx(100.0)


def test_multiple_round_trips_are_logged_and_reconcile_with_equity(tmp_path):
    idx = pd.date_range("2024-01-01", periods=7, freq="B")
    prices = pd.DataFrame(
        {"close": [100.0, 110.0, 110.0, 99.0, 99.0, 108.9, 108.9]},
        index=idx,
    )
    strategy = TimeSignalStrategy(
        StrategyConfig(name="round-trips", universe=["DEMO"], lookback=2),
    ...
    assert result.equity_curve.iloc[-1] == pytest.approx(121000.0)
...
def test_invalid_time_signal_is_rejected(tmp_path):
    ...
    strategy = TimeSignalStrategy(
        StrategyConfig(name="invalid", universe=["DEMO"], lookback=2),
        [0.0, 1.5, 0.0],
    )
...
def test_invalid_price_data_is_rejected(tmp_path):
    ...
    strategy = TimeSignalStrategy(
        StrategyConfig(name="invalid-price", universe=["DEMO"], lookback=2),
        [0.0, 0.0],
    )
...
def test_annualisation_uses_observed_bar_spacing(tmp_path):
    ...
    strategy = TimeSignalStrategy(
        StrategyConfig(name="weekly", universe=["DEMO"], lookback=2),
        [1.0] * 10,
    )
