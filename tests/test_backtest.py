...
def prices():
    ...
    data = pd.DataFrame({"close": np.linspace(100, 120, len(idx))}, index=idx)
    return data
...
def test_backtest_mark_to_market_equity_curve_has_drawdown_and_nonzero_sharpe(tmp_path):
    ...
    result = BacktestEngine(output_dir=tmp_path).run(prices, DummyStrategy(cfg))
...
def test_backtest_time_signal_controls_exposure_path(tmp_path):
    ...
    expected_equity = pd.Series([100000.0, 100000.0, 110000.0, 107800.0, 107800.0], index=idx)
...
def test_backtest_time_signal_midpath_equity_matches_realized_trade_pnl(tmp_path):
    ...
    strategy = TimeSignalStrategy(cfg, [0.0, 1.0, 1.0, 0.0, 0.0])
    result = BacktestEngine(output_dir=tmp_path).run(prices, strategy)
    expected_equity = pd.Series([100000.0, 100000.0, 90000.0, 108000.0, 108000.0], index=idx)
    pd.testing.assert_series_equal(result.equity_curve, expected_equity)
...
def test_backtest_time_signal_losing_trade_sets_zero_win_rate(tmp_path):
    ...
    strategy = TimeSignalStrategy(cfg, [0.0, 1.0, 1.0, 0.0, 0.0])
    result = BacktestEngine(output_dir=tmp_path).run(prices, strategy)
    expected_equity = pd.Series([100000.0, 100000.0, 90000.0, 80000.0, 80000.0], index=idx)
    pd.testing.assert_series_equal(result.equity_curve, expected_equity)
...
def test_backtest_time_signal_partial_exposure_scales_equity_path(tmp_path):
    ...
    assert result.trade_log.loc[0, "entry_date"] == idx[0]
    assert result.trade_log.loc[0, "exit_date"] == idx[-1]
    assert result.trade_log.loc[0, "quantity"] == pytest.approx(500.0)
    assert result.trade_log.loc[0, "pnl_realized"] == pytest.approx(4737.5)
    assert result.trade_log.loc[0, "pnl_unrealized"] == pytest.approx(0.0)
    assert result.trade_log.loc[0, "pnl"] == pytest.approx(4737.5)
...
def test_backtest_time_signal_partial_loss_scales_drawdown_and_return(tmp_path):
    ...
    result = BacktestEngine(output_dir=tmp_path).run(prices, strategy)
    expected_equity = pd.Series([100000.0, 100000.0, 95000.0, 89722.22222222222, 89722.22222222222], index=idx)
    pd.testing.assert_series_equal(result.equity_curve, expected_equity)
    assert result.trade_log.loc[0, "pnl"] == pytest.approx(-10277.77777777778)
    assert result.trade_log.loc[0, "exit_date"] == idx[3]
...
def test_backtest_time_signal_mixed_path_preserves_zero_win_rate_and_drawdown(tmp_path):
    ...
    result = BacktestEngine(output_dir=tmp_path).run(prices, strategy)
    expected_equity = pd.Series([100000.0, 100000.0, 90000.0, 99000.0, 80000.0, 80000.0], index=idx)
    pd.testing.assert_series_equal(result.equity_curve, expected_equity)
    assert result.trade_log.loc[0, "pnl"] == pytest.approx(-20000.0)
...
def test_backtest_trade_log_csv_has_canonical_columns(tmp_path):
    ...
    assert list(trade_log_csv.columns) == ["ticker", "strategy_tag", "status", "entry_date", "entry_price", "exit_date", "exit_price", "side", "quantity", "pnl_realized", "pnl_unrealized", "pnl", "return_pct", "bars"]
...
def test_backtest_metrics_csv_has_canonical_columns(tmp_path):
    ...
    assert list(metrics_csv.columns) == ["total_return", "cagr", "sharpe", "max_drawdown", "win_rate", "profit_factor", "n_trades", "warning_nonpositive_sharpe"]
...
def test_backtest_losing_trade_validates_pnl_fields(tmp_path):
    ...
    prices_down = pd.DataFrame({"close": np.linspace(120, 100, len(idx))}, index=idx)
    cfg = StrategyConfig(name="dummy", universe=list(prices_down.columns), lookback=5)
    result = BacktestEngine(output_dir=tmp_path).run(prices_down, DummyStrategy(cfg))
    assert len(result.trade_log) == 1
    assert result.trade_log.loc[0, "quantity"] == pytest.approx(1.0)
...
def test_backtest_outputs_time_signal_branch():
    ...
    prices = pd.DataFrame({"close": [100.0, 110.0, 120.0, 115.0, 118.0]}, index=pd.to_datetime(["2024-01-01", "2024-01-02", "2024-01-03", "2024-01-04", "2024-01-05"]))
...
def test_backtest_metrics_positive_time_signal_branch():
    ...
    prices = pd.DataFrame({"close": [100.0, 110.0, 120.0, 130.0, 130.0]}, index=pd.to_datetime(["2024-01-01", "2024-01-02", "2024-01-03", "2024-01-04", "2024-01-05"]))
    engine = BacktestEngine()
